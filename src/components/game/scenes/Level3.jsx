...
export default class Level3 extends Phaser.Scene {
    ...
    create() {
        ...
        this.GLITCH_SPEED = 1200; // Slower glitch expansion (was 500)
        this.ROTATION_INTERVAL = 7000;
        this.SHOOT_INTERVAL = 3000; // Slower shooting (was 1500)
        ...
        // Spawn MORE initial glitches for Level 3
        this.spawnInitialGlitches(3);
        ...
        this.time.addEvent({
            delay: 200, // Slower projectiles (was 120)
    ...
    generateGrid() {
        ...
        // Place End in varied locations across the map
        const safeEndSpots = [
            {x: 18, y: 2}, {x: 17, y: 3}, {x: 16, y: 2},
            {x: 2, y: 18}, {x: 3, y: 17}, {x: 4, y: 18},
            {x: 18, y: 10}, {x: 10, y: 2}, {x: 15, y: 15}
        ];
        const endSpot = safeEndSpots[Math.floor(Math.random() * safeEndSpots.length)];
        this.grid[endSpot.y][endSpot.x] = 3;

        // Ensure Path
        this.carvePath({x:8, y:8}, endSpot);
    }
    ...
    shootProjectiles() {
        ...
        // Shoot from 3 random glitches (more than Level 2)
        const shootersCount = Math.min(glitches.length, 3);
        
        // Shuffle glitches to pick random ones
        ...
        for(let i=0; i<shootersCount; i++) {
            ...
            const projSprite = this.add.rectangle(
                ...
                0xff0000 // Red bullet for danger
            );
    ...
    handleWin() {
        this.isWon = true;
        this.game.events.emit('game-over', { win: true, nextLevel: 4 });
        this.scene.pause();
    }
